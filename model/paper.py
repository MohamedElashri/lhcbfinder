...
class PDFCleaner:
    ...
    def clean_pdf_content(content: str, title: str, abstract: str) -> str:
        """
        Clean HTML content by removing redundant information and formatting artifacts.
        ...
        # Try to detect scrambled HTML content
        if len(content) > 100:
            # Calculate ratio of non-ASCII characters to detect papers with encoding issues
            non_ascii_count = sum(1 for char in content if ord(char) > 127)
            if non_ascii_count / len(content) > 0.3:  # More than 30% non-ASCII
                print("Detected possible encoding issues in HTML content")
            ...
            if symbol_count / len(content) > 0.1:  # More than 10% symbols
                print(
                    "High density of special symbols in HTML content, may be poorly extracted math"
                )
        ...
        # content_after_basic = content (Unused)
        ...
        if len(cleaned_content) < 0.3 * original_length:
            print(
                f"Warning: Cleaning removed {original_length - len(cleaned_content)} chars ({(original_length - len(cleaned_content)) / original_length * 100:.1f}% of content)"
            )

            # Try just the basic cleaning first (more conservative)
            basic_cleaned = PDFCleaner._basic_cleaning(
                original_content, title, abstract
            )

            if len(basic_cleaned) >= 0.5 * original_length:
                print("Using basic cleaning instead")
                return basic_cleaned
            else:
                print("Using fallback minimal cleaning")
    ...
    def _fallback_cleaning(content: str, title: str, abstract: str) -> str:
        ...
        if abstract in content and content.count(abstract) > 1:
            ...
            cleaned_content += content[first_pos + len(abstract) :].replace(
                abstract, ""
            )
            content = cleaned_content
    ...
    def chunk_content(content: str, chunk_size: int = 500, overlap: int = 100) -> list:
        ...
        for i in range(0, len(words), chunk_size - overlap):
            ...
            if (
                sentence_end and sentence_end.start() < len(chunk_text) // 3
            ):  # Only if reasonably close to end
...
class Paper:
    def __init__(
        ...
        """
        Initialize a Paper object from a dictionary and optionally load HTML/PDF content.
        Args:
            data_dict: Dictionary containing paper metadata
            html_dir: Optional directory path containing HTML files
            include_content: Whether to include full content in embeddings
        ...
        try:
            ...
            if authors_parsed:
                authors = [
                    author[::-1][1:] for author in authors_parsed
                ]  # Reverses [Last, First, Middle]
        ...
        # Store content
        self._content = ""
        self._content_source = None  # 'html' or 'pdf'

        # Process content if needed
        if include_content:
            # Try HTML first
            if html_dir and self._load_and_clean_html(html_dir):
                self._content_source = "html"
            # Fallback to PDF if available and HTML failed/not provided
            elif pdf_dir and self._load_and_clean_pdf(pdf_dir):
                self._content_source = "pdf"

    def reload_content(
        self, html_dir: Optional[str] = None, pdf_dir: Optional[str] = None
    ):
        """
        Reload content from disk. Useful if content was downloaded after object creation.
        """
        self._content = ""
    ...
    def _load_and_clean_html(self, html_dir: str) -> bool:
        """
        ...
        html_path = os.path.join(html_dir, f"{safe_paper_id}.html")
        ...
        try:
            ...
            cleaned_text = ArxivHTMLParser.clean_html_content(content)
            if cleaned_text and len(cleaned_text) > 100:
                self._content = cleaned_text
                return True
        except Exception as e:
            print(f"Error loading/cleaning HTML for {self.id}: {e}")
        ...
        try:
            ...
            for method in extraction_methods:
                try:
                    ...
                    print(
                        f"Error with extraction method {method.__name__} for {self.id}: {str(e)}"
                    )
                    continue
    ...
    def _extract_with_pypdf2(self, pdf_path):
        ...
        try:
            ...
            print(f"PyPDF2 not available, skipping PDF extraction for {self.id}")
            return None
        ...
        try:
            ...
            try:
                ...
                if (
                    ...
                    print(
                        f"pdftotext failed or returned insufficient content for {self.id}, falling back to PyPDF2"
                    )
            ...
            try:
                ...
                for page_num in range(
                    ...
                    try:
                        ...
                        if error_count < 5:
                            print(
                                f"Page {page_num} extraction error for {self.id}: {str(e)}"
                            )
            except Exception as e:
            ...
            if text and len(text) > 100:
                ...
                cleaned_text = PDFCleaner.clean_pdf_content(
                    text, self.title, self.abstract
                )
                ...
                print(
                    f"Cleaned text for {self.id} was too short ({len(cleaned_text) if cleaned_text else 0} chars), using basic cleaning"
                )
                return (
                    basic_cleaned
                    if basic_cleaned and len(basic_cleaned) >= 100
                    else None
                )
            else:
                print(
                    f"Failed to extract meaningful text from PDF for {self.id} (length: {len(text) if text else 0})"
                )
                return None
        except Exception as e:
    ...
    def has_content(self):
        """Check if the paper has full content loaded."""
        return self._content is not None and len(self._content) > 0

    @property
    def embedding_text_abstract(self):
        """Text used for abstract embedding (primary)."""
        # This one is primarily title + abstract
        return f"Title: {self.title} Year: {self.year} Abstract: {self.abstract}"

    @property
    def embedding_text_full(self):
        """Text used for full content embedding (can be chunked later)."""
        if self._content:
            return self._content
        return self.embedding_text_abstract  # Fallback

    @property
    def metadata(self):
        """Metadata dict that you can store in ChromaDB."""
        return {
            "title": self.title,
